...
# --- Custom Styling Block (Coastal Blue: Deep Teal & Sky Blue Theme) ---
st.markdown(
    """
    <style>
    /* 1. Main Background & Content Styling */
    .stApp {
        background-color: #f0f2f6; /* Soft Off-White Background */
        color: #1c1c1e; /* Dark content color for high contrast */
    }

    /* 2. Streamlit Sidebar Styling (Navigation Bar) */
    [data-testid="stSidebar"] {
        background-color: #F8F8F8; /* Pale Gray Sidebar Background */
        border-right: 2px solid #00474B; /* Deep Teal border for strong separation */
    }
    
    /* Sidebar Title */
    [data-testid="stSidebar"] .st-emotion-cache-10trblm {
        color: #00474B; /* Deep Teal title text */
    }

    /* Sidebar Navigation Links - Text Color */
    .st-ag .st-ah { 
        color: #37474F !important; /* Dark Slate Gray for non-selected links */
    }
    
    /* Sidebar Navigation Links - Selected State */
    .st-ag .st-ag.st-c5.st-c6 { 
        background-color: #E6F0FF; /* Very Pale Blue background for the selected item */
        color: #00474B !important; /* Deep Teal text for selected item */
        border-left: 5px solid #6495ED; /* Highlight with a Sky Blue left border */
        border-radius: 4px;
        padding-left: 10px;
    }

    /* 3. Main Header/Title Styling */
    .st-emotion-cache-10trblm {
        color: #00474B; /* Deep Teal for a strong, professional title */
        font-weight: 700;
        text-shadow: 1px 1px 2px rgba(0,0,0,0.1);
    }

    /* 4. Primary Buttons Styling */
    .stButton>button {
        background-color: #00474B; /* Primary Deep Teal */
        color: white;
        border: none;
        border-radius: 8px;
        padding: 10px 20px;
    ...
    .stButton>button:hover {
        background-color: #003336; /* Darker teal on hover */
        transform: scale(1.02);
    }

    /* 5. Form Submit Buttons (Key Call-to-Action) */
    div[data-testid="stForm"] .stButton>button {
        background-color: #6495ED; /* Sky Blue for high emphasis (e.g., Get Recommendations) */
        color: white;
    }
    ...
    /* 6. Input/Widget Border Styling (Includes all input types: text, file uploader, etc.) */
    .st-emotion-cache-1gsv8h { /* Targets the container of input elements */
        border: 2px solid #ced4da; /* Light gray border for separation */
        border-radius: 8px;
        padding: 10px;
    ...
    /* Text Area/Input Fields inside the container */
    [data-testid="stTextInput"] > div > div > input,
    [data-testid="stTextarea"] > div > div > textarea,
    [data-testid="stFileUploader"] {
        border: 1px solid #ced4da; /* Subtle inner border for the actual field */
        border-radius: 4px;
        padding: 8px 10px;
        background-color: #ffffff;
    }

    /* Input focus effect (Sky Blue highlight when user interacts) */
    .st-emotion-cache-1gsv8h:focus-within {
        border-color: #6495ED; /* Highlight border Sky Blue when focused */
        box-shadow: 0 0 0 0.2rem rgba(100, 149, 237, 0.25); 
    }
    
    /* 7. Dataframes/Tables */
    .st-emotion-cache-1offfwp th {
        background-color: #e8f3f3; /* Very light teal header for tables */
    }
    </style>
    """,
...
# Use this custom markdown for the main title and divider
# Remember to comment out or delete the original st.title("Smart Support AI Dashboard")
st.markdown(
    """
    <h1 style='color: #00474B;'>Smart Support AI Dashboard</h1>
    <hr style='border-top: 2px solid #6495ED;'>
    """,
    unsafe_allow_html=True,
)
